...
class AccountMixin:
    ...
    async def li(self, ctx):
        ...
        try:
            ...
            return await self.error_handler(ctx, e)
        sum_on_hand = sum(search_results[id_legendary_insight].values())
        ...
        total_li = sum_on_hand + sum_crafted
        total_trophies = total_li
        embed = discord.Embed(
            title="{} total Legendary Insights and Divinations earned"
            "".format(total_trophies),
            description="{} on hand, {} used in crafting".format(
                total_trophies, sum_on_hand, sum_crafted),
            color=0x4C139D)
        ...
        if total_li:
            ...
            if sum_perfect_armor:
                value.append(
                    "{} Perfected Envoy Armor Pieces - **{}**").format(
